...
def cra(flow, P, X, index, n_ttl, n_s, thresh=0.01, min_inliers=10000, num_iters=50):
    ...
    for _ in range(num_iters):
        ...
        # Compute sample H matrix (fitting step) with regularization
        #cov_matrix = Xs.T @ Xs
        # Add regularization to prevent singularity
        #regularization = 1e-8 * np.eye(cov_matrix.shape[0])
        #try:
         #   Hs = np.linalg.inv(cov_matrix + regularization) @ (Xs.T @ Ys)
        #except np.linalg.LinAlgError:
        # Fallback to pseudo-inverse if still problematic
         #   Hs = np.linalg.pinv(cov_matrix) @ (Xs.T @ Ys)
