...
Sophus::SE3f Tracking::GrabImageMonocularWithMask(const cv::Mat &im, const cv::Mat &mask, const double &timestamp)
{
    mImGray = im;
    mImMask = mask;  // Store mask
    ...
    else if(mSensor == System::IMU_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            // ========== USE MASK CONSTRUCTOR ==========
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,
                                 mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib,mImMask);
        }
        else
            // ========== USE MASK CONSTRUCTOR ==========
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,
