...
// ========== ADD THIS CONSTRUCTOR (Copy of monocular + mask) ==========
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib, const cv::Mat &mask)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL),mpPrevFrame(pPrevF),mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera),
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false),
     mMask(mask)  // ADD THIS: Store mask
{
    // Frame ID
    mnId=nNextId++;
    ...
    // ========== DS-SLAM FILTERING ==========
    if(!mMask.empty()) {
        FilterDynamicFeatures();  // ERASE dynamic features
    }
    // ========================================
    ...
    // Initialize with CORRECT size (after filtering!)
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
    ...
    if(mbInitialComputations)
    {
        ...
        fy = static_cast<Pinhole*>(mpCamera)->toK().at<float>(1,1);
        cx = static_cast<Pinhole*>(mpCamera)->toK().at<float>(0,2);
        cy = static_cast<Pinhole*>(pCamera)->toK().at<float>(1,2);
        invfx = 1.0f/fx;
        invfy = 1.0f/fy;
...
// ========== ADD THIS METHOD (DS-SLAM ERASURE) ==========
void Frame::FilterDynamicFeatures()
{
    if(mMask.empty() || mvKeys.empty()) return;
    
    std::vector<cv::KeyPoint> staticKeys;
    cv::Mat staticDescs;
    int dynamicCount = 0;
    ...
    for(size_t i = 0; i < mvKeys.size(); i++) {
        ...
        // Check if feature is on dynamic object (white in mask)
        if(x >= 0 && x < mMask.cols && y >= 0 && y < mMask.rows) {
            if(mMask.at<uchar>(y, x) > 200) {  // White = dynamic
                isDynamic = true;
                dynamicCount++;
            }
        }
        ...
        if(!isDynamic) {
            ...
            // Keep corresponding descriptor
            if(!mDescriptors.empty() && i < (size_t)mDescriptors.rows) {
                if(staticDescs.empty()) {
                    mDescriptors.row(i).copyTo(staticDescs);
                } else {
                    cv::vconcat(staticDescs, mDescriptors.row(i), staticDescs);
                }
            }
        }
    }
    
    // ERASE: Replace original data with filtered data
    mvKeys = staticKeys;
    mDescriptors = staticDescs;
    N = mvKeys.size();
    
    std::cout << "Removed " << dynamicCount 
              << " dynamic features, " << N << " static remain" << std::endl;
}
// ========================================================
