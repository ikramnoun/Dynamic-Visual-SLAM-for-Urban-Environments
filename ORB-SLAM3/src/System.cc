...
// ========== ADD THIS METHOD (DS-SLAM mask support) ==========
Sophus::SE3f System::TrackMonocularWithMask(const cv::Mat &im, const cv::Mat &mask, const double &timestamp)
{
    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbShutDown)
    ...
    if(mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocularWithMask but input sensor was not set to Monocular nor Monocular-Inertial." << endl;
        exit(-1);
    }

    cv::Mat imToFeed = im.clone();
    cv::Mat maskToFeed = mask.clone();  // Clone mask
    ...
    if(settings_ && settings_->needToResize()){
        ...
        // Also resize mask to match image
    ...
    // ========== CALL MASK-ENABLED TRACKER ==========
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocularWithMask(imToFeed, maskToFeed, timestamp);
    // ===============================================
